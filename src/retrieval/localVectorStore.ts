...
import type { EmbeddedChunk, RetrievalResult, LegalMetadata } from '../types.js';
  ...
  if (filter) {
    candidates = candidates.filter(chunk => {
      for (const [key, value] of Object.entries(filter)) {
        const metaValue = chunk.metadata[key as keyof LegalMetadata];
